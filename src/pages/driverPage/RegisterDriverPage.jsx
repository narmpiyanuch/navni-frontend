...
const registerDriverSchema = Joi.object({
    ...
    idCard: Joi.string().pattern(/^[0-9]{13}$/).messages({ 'string.pattern.base': '"IdCard" consists of 13 numeric digits to pass the validation' }),
...
export default function RegisterDriverPage() {
    ...
    const labelRegister_1 = [
        { id: 1, title: 'Fist name', value: `${input.firstName}`, name: 'firstName', error: error.firstName || null },
        { id: 2, title: 'Last name', value: `${input.lastName}`, name: 'lastName', error: error.lastName || null },
    ]
    const labelRegister_2 = [
        { id: 4, title: 'ID card', value: `${input.idCard}`, name: 'idCard', error: error.idCard || null },
        { id: 5, title: 'Tel.', value: `${input.phoneNumber}`, name: 'phoneNumber', error: error.phoneNumber || null },
        { id: 6, title: 'Email', value: `${input.email}`, name: 'email', error: error.email || null },
        { id: 7, title: 'Password', value: `${input.password}`, name: 'password', error: error.password || null },
        { id: 8, title: 'Password confirm', value: `${input.confirmPassword}`, name: 'confirmPassword', error: error.confirmPassword || null },
    ]
        ...
        try {
            ...
            const RegisterDriver = async (registerDriver) => {
                try {
                    ...
                    console.log(res)
                    setLoading(true)
                    createAlert('Register Diver SuccessFul!', 'Please, waiting for approved')
                    Navigate(`/login`)
                } catch (err) {
                    createErrorSweetAlert("register Failed", 'Duplicate ID card, Email or Phone number');
                }
            }
            setLoading(true)
            setError({});
                            ...
                            <div className="flex flex-col gap-2 pt-12">
                                {labelRegister_1.map(el =>
                                    <div key={el.id} className="flex flex-col gap-[0.5px] items-end pr-2 " >
                                        <InputRegisterForm title={el.title} value={el.value} name={el.name} onChange={handdleChangInput} error={el.error} />
