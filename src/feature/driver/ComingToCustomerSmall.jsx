...
export default function ComingToCustomerSmall({ bookingItem }) {
  const { pickupUserForDriver } = useDriver();
  const { getBookingItemForDriver } = useBooking();
  const { getAcceptBookingItemForDriver, getPickedBookingItemForDriver } =
    useDriver();

  const handlePickupUser = async (e) => {
    try {
      e.preventDefault();
