                        ...
                        <div className="fixed top-[30%] items-center justify-start bg-MonoColor-50 h-full w-screen rounded-[20px_20px_0px_0px] overflow-hidden transit ">
                            ...
                            <div className='flex items-center justify-center h-[140px] bg-MonoColor-50 gap-2'>
                                ...
                                <div className='flex flex-col items-start justify-center bg-Primary-lightest w-[240px] h-[100px] rounded-3xl gap-2'>
                                    <input placeholder='From' className='outline-none text-[18px] font-semibold text-MonoColor-400 bg-transparent pl-4 active:text-MonoColor-700'></input>
                                    <hr className='border-2 border-Primary-light w-[240px]' />
                                    <input placeholder='To' className=' outline-none text-[18px] font-semibold text-MonoColor-400 bg-transparent pl-4 active:text-MonoColor-700'></input>
                                </div>
                            </div>
                            <div>
                                <p className='text-[14px] text-MonoColor-400 pl-4'>All hop point</p>
                                {namePin.map(el => <div key={el.id} className='flex justify-between px-4 py-2 items-center'>
                                    <button className='flex items-end gap-2'>
                                        <img src={purplePin} alt='pin' className='w-[32px]' />
                                        <p className='text-[16px] text-MonoColor-700 active:text-MonoColor-300'>{el.title}</p>
                                    </button>
                                    <p className='text-[16px] text-MonoColor-400'> {el.distance}</p>
                                </div>)}
                            </div>
                        </div>
                    </div>
